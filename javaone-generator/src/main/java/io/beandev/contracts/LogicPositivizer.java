...
public class LogicPositivizer {
    public static void main(String[] args) {
        ...
        SourceRoot sourceRoot = new SourceRoot(CodeGenerationUtils.mavenModuleRoot(LogicPositivizer.class).resolve("../resources/main"));

        // Our sample is in the root of this directory, so no package name.
        CompilationUnit cu = sourceRoot.parse("", "Blabla.java");

        for (MethodDeclaration method : cu.findAll(MethodDeclaration.class)) {
            // Print the method's signature
            System.out.println("Method: " + method.getSignature());

            // Get and print the comment, if it exists
            Comment comment = method.getComment().orElse(null);
            if (comment != null) {
                System.out.println("Comment: " + comment.getContent());
            } else {
                System.out.println("No comment found for this method");
            }
        }
